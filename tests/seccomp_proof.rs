...
// We replicate the BPF simulation logic here so the test is self-contained.
...
fn simulate_bpf(allowed: &[i32], syscall_num: i32) -> u32 {
    ...
    let mut insns: Vec<(u16, u8, u8, u32)> = vec![(BPF_LD_W_ABS, 0, 0, 0)]; // load syscall number
    for i in 0..n {
    ...
    loop {
        ...
        match code {
            BPF_LD_W_ABS => {
                a = k; // syscall number is already loaded in 'a'
                pc += 1;
            }
            BPF_JMP_JEQ_K => {
                if a == k {
